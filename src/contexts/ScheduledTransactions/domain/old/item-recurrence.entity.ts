...
export class ItemRecurrence {
		...
		if (n.value === 1) {
			...
			for (let i = 1; i < n.value; i++) {
				untilDate = this.calculateNextDate(
					new ScheduledTransactionDate(untilDate.value),
					frequency
				);
			}
		}
	...
	static calculateNextDate(
		actualDate: ScheduledTransactionDate,
		frequency: ItemRecurrenceFrequency
	...
	validateScheduleChange(
		...
		if (this.isOneTime()) {
			...
			if (untilDate) {
				...
				let currentDate = new ScheduledTransactionDate(startDate);
	...
	createRecurrences(
		...
		if (this.isOneTime()) {
			this._recurrences.push(
				new ItemRecurrenceInfo(
					new ScheduledTransactionDate(this._startDate),
		...
		let nextDate = new ScheduledTransactionDate(this._startDate);
	...
	get totalRecurrences(): number {
		...
		let nextDateTime = new ScheduledTransactionDate(
			this.startDate
		).getTime();
		const untilDateTime = this.untilDate.getTime();
		let count = 0;
		let currentDate = new ScheduledTransactionDate(this.startDate);
